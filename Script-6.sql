...
SELECT * FROM Books WHERE Genre = 'Khoa học';

SELECT * FROM Members WHERE City = 'Hanoi';
...
SELECT m.Name, COUNT(b.BorrowID) AS SoSachMuon
FROM Members m
JOIN Borrowings b
ON m.MemberID = b.MemberID
GROUP BY m.MemberID, m.Name
...
SELECT m.Name, SUM(DATEDIFF(b.ReturnDate, b.BorrowDate)) AS TongSoNgayGiu
FROM Members m
JOIN Borrowings b
ON m.MemberID = b.MemberID
...
SELECT m.Name, bk.Title, b.BorrowDate
FROM Members m
JOIN Borrowings b
ON m.MemberID = b.MemberID
JOIN Books bk
...
SELECT bk.Genre, COUNT(b.BorrowID) AS SoLanMuon
FROM Books bk
JOIN Borrowings b
